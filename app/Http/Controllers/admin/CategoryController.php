...
use App\Exports\SubCategorysExport;
use PDF;
...
class CategoryController extends Controller
{
    ...
    {
        $subcategory = SubCategory::where('user_id',Auth::user()->id)
        ->get();
       ...
       /* foreach ($category as $value) {
            $value->cat = Category::where('id',$value->category_id)->first();
            echo "<pre>";
            print_r($value->cat);
            echo "</pre>";
            exit();*/
            /*if($value->cat){
                $value->cat =$subcatname->name;
            }else{
                $value->subcatname='No cat';
            }*/
       // }

        return view('admin.subcategory.index', ['collection' => $subcategory]);
    }
    ...
    {
        return Excel::download(new CategorysExport, 'categorys-collection.csv');
    }
    public function subfileExport()
    {
        return Excel::download(new SubCategorysExport, 'subcategorys-collection.csv');
    }

    // Generate PDF
    public function CategoryCreatePDF() 
    {
      ...
      $pdf = PDF::loadView('admin.pdf.categorys_pdf', $data);

      // download PDF file with download method
      return $pdf->download('category.pdf');
    }

    public function subCategoryCreatePDF() 
    {
      $subcategory = SubCategory::where('user_id',Auth::user()->id)
        ->get();
        foreach($subcategory as $value)
     ...
     // $data = Subcategory::where('user_id',Auth::user()->id)->get();

      // share data to view
      view()->share('subcategorys',$subcategory);
      $pdf = PDF::loadView('admin.pdf.subcategorys_pdf', $subcategory);
